...
class NowPlayingCentral: NSObject {
    ...
    private var players: [MIDIPlayer] = []
    private var activePlayer: MIDIPlayer? {
...
//        MPRemoteCommandCenter.shared().previousTrackCommand.addTarget(self, action: #selector(previousTrackCommand(event:)))
    ...
    func makeActive(player: MIDIPlayer) {
    ...
    func removeFromPlayers(player: MIDIPlayer?) {
    ...
    func initNowPlayingInfo(for midiPlayer: MIDIPlayer) {
    ...
    func updateNowPlayingInfo(for midiPlayer: MIDIPlayer, with updatedDict: [String: Any]) {
